...
% load first file
f = dir(fullfile('_res','_no_pad',sprintf('%s_*g%02d*alpha%02d*.mat',simtype,gc(1)*10,alpha*100)));
...
for i = 1:length(gc)
    ...
    % build file list
    files = dir(fullfile('_res','_no_pad',sprintf('%s_*g%02d*alpha%02d*.mat',simtype,gc(i)*10,alpha*100)));
    ...
    for j = 1:length(files)
        ...
        w(:,j,i) = mean(res.wm,1);
        g(:,j,i) = res.neuron.g(1:size(res.wm,2));
...
for i = 1:length(gc)
    ...
    patchErrorBars(1:size(res.wm,2),squeeze(w(:,:,i))',cc(i,:),'prctile');
end
...
saveFigPDF(f1,[900 900],sprintf('./_plots/_%s_res_nopad_alpha%03d.pdf', ...
                                simtype,alpha*100))